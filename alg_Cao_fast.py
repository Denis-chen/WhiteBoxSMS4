...
def or_constructor(or_table):
    for r in xrange(32):
        ...
        for i in xrange(8):
            or_table[r].append([])
            or_table[r][i] = []
            nonlinear(or_table[r][i], 4, 4, False)
...
def pr_constructor(pr_table):
    for r in xrange(32):
        ...
        for i in xrange(8):
            pr_table[r].append([])
            pr_table[r][i] = []
            nonlinear(pr_table[r][i], 4, 4, True)


def pr_inv_constructor(pr_inv_table, pr_table, qr_table):
    for r in xrange(32):
        pr_inv_table.append([])
        for i in xrange(8):
            pr_inv_table[r].append([])
            for m in xrange(16):
                pr_inv_table[r][i].append([])
                for n in xrange(16):
                    m_inv = 0
                    while pr_table[r][i][m_inv] != m:
                        m_inv += 1
                    pr_inv_table[r][i][m].append(m_inv ^ qr_table[r][i][n])
...
# global qr_table
qr_table = []
or_constructor(qr_table)

# global pr_inv_table
pr_inv_table = []
pr_inv_constructor(pr_inv_table, pr_table, qr_table)
...
def wb_crypt(pt):
    ...
    for each in text:
        ...
        for r in xrange(32):
            ...
            X_r_0 = x[0]
            for i in xrange(8):
                X_r_0 = X_r_0[0:i] + tr.Int2HexStr(pr_table[r][i][tr.HexStr2Int(X_r_0[i])], 1) + X_r_0[i + 1:]
            ...
            new_x = ''
            for i in xrange(8):
                new_x += tr.Int2HexStr(pr_inv_table[r][i][tr.HexStr2Int(X_r_0[i])][tr.HexStr2Int(y[i])], 1)
        ...
        for r in xrange(32):
            ...
            X_r_0 = x[0]
            for i in xrange(8):
                X_r_0 = X_r_0[0:i] + tr.Int2HexStr(pr_table[31 - r][i][tr.HexStr2Int(X_r_0[i])], 1) + X_r_0[i + 1:]
            ...
            new_x = ''
            for i in xrange(8):
                new_x += tr.Int2HexStr(pr_inv_table[31 - r][i][tr.HexStr2Int(X_r_0[i])][tr.HexStr2Int(y[i])], 1)
